...
CodeFlask.prototype.renderOutput = function(highlightCode, input) {
    ...
    this.inputListeners.map(function(cb) {
      cb(input.value) // Pass the original value
    })
}
...
CodeFlask.prototype.handleInput = function(textarea, highlightCode, highlightPre) {
    ...
    var shortPair = ['php']
    var shortPairWithAttr = ['loop','if','is','for','related','users']
    var shortSingle = ['field','taxonomy','each','user']
    var tagPair = ['h1','h2','h3','h4','b','ul','ol','li','script','style']
    var tagPairWithAttr = ['div','a']
    var tagSingle = ['br','hr']
...
//console.log(e.keyCode)
        ...
        if (e.keyCode === 9){
          ...
          if (shortPair.indexOf(tag)>=0) {
            // Shortcode pair
            insertVal = '['+tag+'][/'+tag+']'
          } else if (shortPairWithAttr.indexOf(tag)>=0) {
            // Shortcode pair with attributes
            insertVal = '['+tag+' ][/'+tag+']'
          } else if (shortSingle.indexOf(tag)>=0) {
            // Shortcode single
            insertVal = '['+tag+' ]'
          } else if (tagPair.indexOf(tag)>=0) {
            // Tag pair
            insertVal = '<'+tag+'></'+tag+'>'
          } else  if (tagPairWithAttr.indexOf(tag)>=0) {
            // Tag pair with attributes
            insertVal = '<'+tag+' ></'+tag+'>'
          } else if (tagSingle.indexOf(tag)>=0) {
            // Tag single
            insertVal = '<'+tag+'>'
          } else {
            // Soft tab: 2 spaces
